...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  // If console is enabled, it will print the matrix position and status of each key pressed
    ...
    switch (keycode) {
        ...
        case PSS_NEXT:
            if (record->event.pressed) {
            SEND_STRING(SS_DOWN(X_ROPT) SS_DOWN(X_LCMD) SS_TAP(X_DOWN) SS_UP(X_ROPT) SS_UP(X_LCMD)); // ctrl shift d
            }
            break;
        case PSS_PREV:
            if (record->event.pressed) {
            SEND_STRING(SS_DOWN(X_ROPT) SS_DOWN(X_LCMD) SS_TAP(X_UP) SS_UP(X_ROPT) SS_UP(X_LCMD)); // ctrl shift d
            }
            break;
    }
    return true;
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [_RAISE] = LAYOUT_5x6_dart(
       ...
       _______,_______,_______,_______,_______,KC_LBRC,                        _______,KC_MS_BTN1,KC_MS_BTN2 ,KC_SLCK,KC_MUTE,_______,
       _______,KC_LEFT,KC_UP  ,KC_DOWN,KC_RGHT,KC_LPRN,                        KC_MS_LEFT,KC_MS_DOWN,KC_MS_UP,KC_MS_RIGHT,KC_MNXT,KC_VOLU,
